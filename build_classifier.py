...
def build_classifier(train_data_x, train_data_y):
    ...
    classifier = svm.SVC()
    print "svc built, doing training"
    classifier.fit(train_data_x, train_data_y)
    print "Doing classifier estimation."
    return classifier

def show_usage():
    print 'build_classifier.py -i <input_directory> -o <output_directory>'
...
def main(argv):
    ...
    train_set = numpy.random.uniform(0,1,len(data_x)) <= 0.8
    ...
    joblib.dump(classifier, directory_to_write+'/classifier_out.pkl')

    numpy.set_printoptions(threshold=numpy.nan)
    "Doing re-prediction on training class for testing"
